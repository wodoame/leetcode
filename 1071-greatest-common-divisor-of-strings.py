...
def gcdOfStrings(str1, str2):
    ...
    # Since we want the largest common divisor we start by considering the while string (the smaller one)
    str1_len = len(str1)
    str2_len = len(str2)
    min_len = min(str1_len, str2_len) 
    for i in range(min_len, 0, -1):
        # If the length of the word (i) is not divisible by either str1 or str2 (their lengths) ...
